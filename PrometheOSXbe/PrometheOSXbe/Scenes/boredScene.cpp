...
boredScene::boredScene()
{
	mCounter = 0;
	restart();
}

void boredScene::update()
{
	// Exit Action
	...
	if (mIsGameOver == true && mPlayerScore > settingsManager::getPlayerHiScore())
	{
		settingsManager::setPlayerHiScore(mPlayerScore);
	}
}
...
void boredScene::render()
{
	...
	char* score = stringUtility::formatString("Hi-Score: %i, Score: %i", settingsManager::getPlayerHiScore(), mPlayerScore);
