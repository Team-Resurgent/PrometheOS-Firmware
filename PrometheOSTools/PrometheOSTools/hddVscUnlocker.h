...
protected:
	...
	bool sgVscEnDiagMode();
	bool sgVscDiagSendCmd(char* cmd);
	bool sgVscDiagRead(char* buf, uint32_t timeout = 10000);
	bool sgVscDiagReadWait(uint8_t* status, uint32_t timeout = 10000);
	bool sgVscDiagSendByte(uint8_t cmdByte);
	bool sgVscDiagReadByte();
	bool sgVscDiagExecCmd(char* cmd, char* buffer, uint32_t timeout = 10000);
