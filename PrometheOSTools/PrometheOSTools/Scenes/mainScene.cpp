...
void mainScene::update()
{
	...
	if (inputManager::buttonPressed(ButtonA))
	{
		...
		{
			sceneManager::pushScene(sceneItemSystemToolsScene);
			return;
		}
		else if (mSelectedControl == 1) 
		{
			...
			{
				context::setModchip(new modchipXecuter(), modchipTypeXecuter);
			}
			else if (context::getModchipType() == modchipTypeXecuter)
			{
				context::setModchip(new modchipAladdin1mb(), modchipTypeAladdin1mb);
			}
			else if (context::getModchipType() == modchipTypeAladdin1mb)
			{
				context::setModchip(new modchipXchanger(), modchipTypeXchanger);
			}
			else if (context::getModchipType() == modchipTypeXchanger)
			{
				context::setModchip(new modchipXenium(), modchipTypeXenium);
			}
			return;
		}
...
void mainScene::render()
{
	...
	component::button(mSelectedControl == 0, false, "System Settings", 193, yPos, 322, 30);

	yPos += 40;

	if (context::getModchipType() == modchipTypeXenium)
	{
		component::button(mSelectedControl == 1, false, "Modchip: Xenium", 193, yPos, 322, 30);
	}
	else if (context::getModchipType() == modchipTypeXecuter)
	{
		component::button(mSelectedControl == 1, false, "Modchip: Xecuter", 193, yPos, 322, 30);
	}
	else if (context::getModchipType() == modchipTypeAladdin1mb)
	{
		component::button(mSelectedControl == 1, false, "Modchip: Aladdin 1MB", 193, yPos, 322, 30);
	}
	else if (context::getModchipType() == modchipTypeXchanger)
	{
		component::button(mSelectedControl == 1, false, "Modchip: Xchanger", 193, yPos, 322, 30);
	}

	yPos += 40;

	component::button(mSelectedControl == 2, false, "Flash Tools", 193, yPos, 322, 30);
